...
const CandidatesGrid = () => {
  ...
  const { data: candidates, isLoading, error } = useGetJobCandidates()
  console.log(candidates)

  if (isLoading) return <p>Loadubg...</p>
  if (error) {
      ...
      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {candidates?.map((candidate: User) => (
          <div className="bg-white flex flex-col gap-2  rounded p-4 ring-1 ring-gray-200 shadow" key={candidate._id}>
            ...
            <div className="">
              <p className='text-[#797979] text-sm flex gap-1 items-center'><PiSuitcase className='mr-2' size={16} color='155DFC' /><span className='font-semibold text-[#111]'>{candidate.applications?.length}</span> applications</p>
            </div>
          </div>
        ))
        }
