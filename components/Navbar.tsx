  ...
  return (
    <nav className='relative flex md:w-[85%] md:mx-auto justify-between items-center px-20 py-4'>
      ...
      <div className="hidden md:block">
        {isAuthed
          ? <Link href={'/dashboard'}> <button className='bg-[#1B1F87] font-semibold rounded cursor-pointer  text-white px-6 py-3'>Dashboard</button></Link>
      ...
      {isMenuOpen && (
        <div className='absolute top-full px-6 py-4 right-0 shadow flex flex-col gap-4'>
            <Link href={'/'} className={pathname === "/" ? "text-[#1b1f87a0] font-bold" : ""}>Home</Link>
            <Link href={'/vacancies'} className={pathname === "/vacancies" ? "text-[#1b1f87a0] font-bold" : ""}>Vacancies</Link>
            <Link href={'/contact'} className={pathname === "/contact" ? "text-[#1b1f87a0] font-bold" : ""}>Contact</Link>
          <div className="md:block">
            {isAuthed
              ? <Link href={'/dashboard'}> <button className='bg-[#1B1F87] font-semibold text-sm rounded cursor-pointer  text-white px-5 py-2'>Dashboard</button></Link>
