    ...
    return (
        <form onSubmit={handleSubmit} className="w-full max-w-xl rounded bg-[#335F84] px-6 py-10 text-white md:px-12" aria-labelledby="contact-form-title" aria-busy={submit.isPending}>
            ...
            <div>
                ...
                <input required minLength={2} maxLength={120} autoComplete="name" type="text" id="contact-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="w-full bg-white px-3 py-2 rounded text-black" />
            </div>
            <div>
                <label className="block my-4 text-lg font-medium" htmlFor="contact-email">Email</label>
                <input required maxLength={320} autoComplete="email" type="email" id="contact-email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} className="w-full bg-white px-3 py-2 rounded text-black" />
            </div>
            <div>
                <label className="block my-3 text-lg font-medium" htmlFor="contact-subject">Subject</label>
                <input required minLength={3} maxLength={200} type="text" id="contact-subject" value={form.subject} onChange={(event) => setForm({ ...form, subject: event.target.value })} className="w-full bg-white px-3 py-2 rounded text-black" />
            </div>
            <div>
                <label className="block my-3 text-lg font-medium" htmlFor="contact-message">Message</label>
                <textarea required minLength={10} maxLength={5000} id="contact-message" rows={5} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} className="bg-white px-3 py-2 w-full rounded text-black" />
            </div>
